...
# Loop to restart tshark periodically to clear memory
while true; do
    # Capture all traffic with DNS details
    # -n: Disable name resolution (Saves HUGE amount of RAM)
    # -a duration:60: Restart every 60s to clear memory
    tshark -i "$IFACE" -l -n -a duration:60 -t ad -T fields -E separator='|' \
        -e frame.time \
        -e ip.src \
        -e ip.dst \
        -e _ws.col.Protocol \
    ...
    # Brief pause to prevent CPU spike if tshark fails immediately
    sleep 0.5
done
